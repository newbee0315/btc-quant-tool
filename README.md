...
### 最新特性 (New Features)
*   **多币种支持**: 扩展支持 Top 30 主流币种（BTC, ETH, SOL, BNB, DOGE, XRP 等）的实时监控与交易。
*   **激进排行榜交易 (Aggressive Leaderboard Trading)**: 自动扫描市场波动率最高的币种，在确定性极高时（预测置信度 > 0.8）进行实时小仓位介入。
*   **智能防限流**: 实现了 `Slow Fetch` 机制，分批次（每 2 小时）轮询 30 个币种的数据，避免触发交易所 API 频率限制。
*   **动态风控**: 
