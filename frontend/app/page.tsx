...
export default function Home() {
    ...
    useEffect(() => {
        fetchStatus();
        const interval = setInterval(fetchStatus, 3000); // Poll every 3 seconds for faster updates
        return () => clearInterval(interval);
    }, []);
    ...
    useEffect(() => {
        ...
        const interval = setInterval(fetchStrategySignals, 5000); // Poll every 5 seconds for faster signals
        return () => clearInterval(interval);
    }, []);
    ...
    useEffect(() => {
        ...
        const interval = setInterval(fetchData, 5000); // Poll every 5s for chart data updates
    ...
    const pnlColor = (trader?.stats.total_pnl || 0) >= 0 ? 'text-green-600 dark:text-[#0ECB81]' : 'text-red-600 dark:text-[#F6465D]';
    const pnlSign = (trader?.stats.total_pnl || 0) >= 0 ? '+' : '';
    const unrealizedPnlColor = (trader?.unrealized_pnl || 0) >= 0 ? 'text-green-600 dark:text-[#0ECB81]' : 'text-red-600 dark:text-[#F6465D]';
    ...
    const roiColor = roi >= 0 ? 'text-green-600 dark:text-[#0ECB81]' : 'text-red-600 dark:text-[#F6465D]';
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-[#0E1117] text-gray-900 dark:text-[#FAFAFA] font-sans flex flex-col">
            {/* Header */}
            <header className="border-b border-gray-200 dark:border-[#2B3139] px-4 py-3 md:px-6 md:py-4 flex justify-between items-center bg-white dark:bg-[#161A25]">
                <div className="flex items-center gap-2 md:gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] md:text-xs text-gray-500 dark:text-[#848E9C]">AI-Powered Market Analysis</p>
                    </div>
                </div>
                <div className="flex items-center gap-2 md:gap-4">
                    ...
                    <div className="flex flex-col items-end group relative mr-2">
                        <div className="flex items-center gap-1.5 cursor-help">
                            ...
                            <span className={`text-xs font-medium ${
                                trader?.connection_status === 'Connected' ? 'text-green-600 dark:text-[#0ECB81]' : 
                                trader?.connection_status === 'Error' ? 'text-red-600 dark:text-[#F6465D]' : 'text-yellow-600 dark:text-[#F0B90B]'
                            }`}>
                        ...
                        {trader?.connection_error && (
                            <div className="absolute top-full right-0 mt-2 p-2 bg-white dark:bg-[#1E2329] border border-red-900/50 rounded shadow-xl text-xs text-red-600 dark:text-[#F6465D] w-64 z-50 hidden group-hover:block">
                                {trader.connection_error}
                            </div>
                        )}
                    </div>
                    ...
                    <button 
                        ...
                        className="flex items-center gap-2 px-2 py-1.5 md:px-3 bg-gray-100 dark:bg-[#2B3139] hover:bg-gray-200 dark:hover:bg-[#363C45] text-gray-900 dark:text-[#EAECEF] text-xs md:text-sm font-medium rounded-lg transition-colors border border-gray-300 dark:border-[#474D57]"
                    >
                        <Beaker className="w-3 h-3 md:w-4 md:h-4 text-yellow-600 dark:text-[#F0B90B]" />
                    ...
                    <button 
                        ...
                        className="flex items-center gap-2 px-2 py-1.5 md:px-3 bg-gray-100 dark:bg-[#2B3139] hover:bg-gray-200 dark:hover:bg-[#363C45] text-gray-900 dark:text-[#EAECEF] text-xs md:text-sm font-medium rounded-lg transition-colors border border-gray-300 dark:border-[#474D57]"
                    >
                        <LayoutGrid className="w-3 h-3 md:w-4 md:h-4 text-yellow-600 dark:text-[#F0B90B]" />
                        <span className="hidden sm:inline">Portfolio</span>
                    </button>
                    <a href="/data" className="hidden md:block text-sm text-gray-500 dark:text-[#848E9C] hover:text-yellow-600 dark:hover:text-yellow-600 dark:hover:text-yellow-600 dark:text-[#F0B90B] transition-colors">
                        Data Info
                    </a>
                    <a href="/models" className="hidden md:block text-sm text-gray-500 dark:text-[#848E9C] hover:text-yellow-600 dark:hover:text-yellow-600 dark:hover:text-yellow-600 dark:text-[#F0B90B] transition-colors">
                        Model Info
                    </a>
                    <ThemeToggle />
                    <div className="flex items-center gap-2 px-2 py-1 md:px-3 md:py-1.5 bg-white dark:bg-[#1E2329] rounded-full border border-gray-200 dark:border-[#2B3139]">
                        <span className="w-1.5 h-1.5 md:w-2 md:h-2 bg-[#0ECB81] rounded-full animate-pulse"></span>
                        <span className="text-[10px] md:text-xs font-medium text-green-600 dark:text-[#0ECB81] whitespace-nowrap">
            ...
            <main className="flex-1 p-4 md:p-6 space-y-6 overflow-y-auto bg-gray-50 dark:bg-[#161A1E]">
                {/* Top Metrics Row */}
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
                    <div className="lg:col-span-4 h-[160px]">
                        <MonitoredTickers />
                    </div>
                    <div className="lg:col-span-1 h-[160px] bg-white dark:bg-[#1E2329] p-4 rounded-xl border border-gray-200 dark:border-[#2B3139] shadow-sm hover:border-gray-300 dark:border-[#474D57] transition-colors flex flex-col justify-center">
                        <p className="text-gray-500 dark:text-[#848E9C] text-xs font-medium uppercase mb-3">Fear & Greed Index</p>
                        <div className="flex flex-col gap-2">
                             <span className={`text-4xl font-bold ${
                                fng ? (parseInt(fng.value) > 50 ? 'text-green-600 dark:text-[#0ECB81]' : 'text-red-600 dark:text-[#F6465D]') : 'text-yellow-600 dark:text-[#F0B90B]'
                            }`}>
                                {fng ? fng.value : '---'}
                            </span>
                            <span className={`text-sm font-medium px-2 py-1 rounded w-fit ${
                                fng ? (parseInt(fng.value) > 50 ? 'bg-green-50 dark:bg-[#0ECB81]/10 text-green-600 dark:text-[#0ECB81]' : 'bg-red-50 dark:bg-[#F6465D]/10 text-red-600 dark:text-[#F6465D]') : 'bg-yellow-50 dark:bg-[#F0B90B]/10 text-yellow-600 dark:text-[#F0B90B]'
                            }`}>
                                {fng ? fng.value_classification : 'Loading...'}
                            </span>
                        </div>
                        <p className="text-[10px] text-gray-500 dark:text-[#848E9C] mt-4">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
                    ...
                    {equityHistory.length > 0 ? (
                        <div className="h-[300px]">
                         ...
                         <div className="h-[300px] bg-white dark:bg-[#1E2329] rounded-xl border border-gray-200 dark:border-[#2B3139] flex items-center justify-center text-gray-500 dark:text-[#848E9C]">
                ...
                <div className="mt-6">
                    <div className="flex items-center justify-between mb-4 px-1">
                        <h2 className="text-xl font-bold flex items-center gap-2 text-gray-900 dark:text-[#EAECEF]">
                            <LayoutGrid className="w-5 h-5 text-yellow-600 dark:text-[#F0B90B]" />
                            Dashboard Overview
                        </h2>
                        <span className="text-xs text-gray-500 dark:text-[#848E9C] bg-white dark:bg-[#1E2329] px-2 py-1 rounded border border-gray-200 dark:border-[#2B3139]">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
                        {/* Total Capital */}
                        <div className="bg-white dark:bg-[#1E2329] p-4 rounded-xl border border-gray-200 dark:border-[#2B3139] relative overflow-hidden shadow-md hover:scale-[1.02] transition-transform duration-200">
                            <div className="absolute top-0 right-0 p-3 opacity-10">
                                <DollarSign className="w-12 h-12 text-gray-900 dark:text-[#EAECEF]" />
                            </div>
                            <p className="text-gray-500 dark:text-[#848E9C] text-xs font-medium uppercase mb-2">Total Equity</p>
                            <h3 className="text-2xl font-bold font-mono tracking-tight text-gray-900 dark:text-[#EAECEF]">
                                ${trader?.equity ? trader.equity.toFixed(2) : '---'}
                            </h3>
                            <p className="text-xs text-gray-500 dark:text-[#848E9C] mt-1">
                        ...
                        <div className="bg-white dark:bg-[#1E2329] p-4 rounded-xl border border-gray-200 dark:border-[#2B3139] relative overflow-hidden shadow-md hover:scale-[1.02] transition-transform duration-200">
                            <div className="absolute top-0 right-0 p-3 opacity-10">
                                <Wallet className="w-12 h-12 text-gray-900 dark:text-[#EAECEF]" />
                            </div>
                            <p className="text-gray-500 dark:text-[#848E9C] text-xs font-medium uppercase mb-2">Total Position Value</p>
                            <h3 className="text-2xl font-bold font-mono tracking-tight text-yellow-600 dark:text-[#F0B90B]">
                        ...
                        <div className="bg-white dark:bg-[#1E2329] p-4 rounded-xl border border-gray-200 dark:border-[#2B3139] relative overflow-hidden shadow-md hover:scale-[1.02] transition-transform duration-200">
                            <div className="absolute top-0 right-0 p-3 opacity-10">
                                <Activity className="w-12 h-12 text-gray-900 dark:text-[#EAECEF]" />
                            </div>
                            <p className="text-gray-500 dark:text-[#848E9C] text-xs font-medium uppercase mb-2">Unrealized PnL</p>
                        ...
                        <div className="bg-white dark:bg-[#1E2329] p-4 rounded-xl border border-gray-200 dark:border-[#2B3139] relative overflow-hidden shadow-md hover:scale-[1.02] transition-transform duration-200">
                            <div className="absolute top-0 right-0 p-3 opacity-10">
                                <TrendingUp className="w-12 h-12 text-gray-900 dark:text-[#EAECEF]" />
                            </div>
                            <p className="text-gray-500 dark:text-[#848E9C] text-xs font-medium uppercase mb-2">Total Realized PnL</p>
                            ...
                            <p className="text-xs text-gray-500 dark:text-[#848E9C] mt-0.5">
                        ...
                        <div className="bg-white dark:bg-[#1E2329] p-4 rounded-xl border border-gray-200 dark:border-[#2B3139] relative overflow-hidden shadow-md hover:scale-[1.02] transition-transform duration-200">
                            <div className="absolute top-0 right-0 p-3 opacity-10">
                                <CheckCircle2 className="w-12 h-12 text-gray-900 dark:text-[#EAECEF]" />
                            </div>
                            <p className="text-gray-500 dark:text-[#848E9C] text-xs font-medium uppercase mb-2">Win Rate (All Time)</p>
                            <h3 className="text-2xl font-bold font-mono tracking-tight text-yellow-600 dark:text-[#F0B90B]">
                        ...
                        <div className="bg-white dark:bg-[#1E2329] p-4 rounded-xl border border-gray-200 dark:border-[#2B3139] relative overflow-hidden shadow-md hover:scale-[1.02] transition-transform duration-200">
                            <div className="absolute top-0 right-0 p-3 opacity-10">
                                <List className="w-12 h-12 text-gray-900 dark:text-[#EAECEF]" />
                            </div>
                            <p className="text-gray-500 dark:text-[#848E9C] text-xs font-medium uppercase mb-2">Open Orders</p>
                            <h3 className="text-2xl font-bold font-mono tracking-tight text-gray-900 dark:text-[#EAECEF]">
                    ...
                    <div className="bg-white dark:bg-[#1E2329] p-4 rounded-xl border border-gray-200 dark:border-[#2B3139] relative overflow-hidden mb-6">
                        <div className="text-gray-500 dark:text-[#848E9C] text-xs font-medium uppercase mb-4 flex items-center gap-3">
                            ...
                            <span className="hidden md:flex items-center gap-2 ml-2 normal-case">
                                <span className="text-[10px] text-gray-500 dark:text-[#848E9C]">Lev</span>
                                <span className="text-[10px] font-mono text-gray-900 dark:text-[#EAECEF]">{leverageValue}x</span>
                                <input
                                    type="range"
                                    min={1}
                                    max={10}
                                    ...
                                    className="w-40 h-2 accent-yellow-600 dark:accent-[#F0B90B]"
                                ...
                                <input
                                    ...
                                    className="w-12 bg-gray-50 dark:bg-[#0E1117] border border-gray-200 dark:border-[#2B3139] rounded px-1 py-0.5 text-[10px] text-center text-gray-900 dark:text-[#EAECEF]"
                                ...
                                <button
                                    ...
                                    disabled={savingLev}
                                    className={`px-2 py-0.5 rounded text-[10px] ${savingLev ? 'bg-gray-100 dark:bg-[#2B3139] text-gray-500 dark:text-[#848E9C]' : 'bg-[#474D57] hover:bg-[#59606D] text-white'}`}
                                    aria-label="Save Leverage"
                                >
                                    {savingLev ? '...' : 'Save'}
                                </button>
                                {levMsg && <span className="text-[10px] text-gray-500 dark:text-[#848E9C]">{levMsg}</span>}
                            </span>
                        </div>
                        ...
                        {trader?.positions && Object.keys(trader.positions).length > 0 ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {Object.entries(trader.positions)
                                    ...
                                    .map(([symbol, pos]) => {
                                    const posPnlColor = pos.unrealized_pnl >= 0 ? 'text-green-600 dark:text-[#0ECB81]' : 'text-red-600 dark:text-[#F6465D]';
                                        ...
                                        <div key={symbol} className="flex flex-col gap-2 bg-gray-100 dark:bg-[#2B3139]/30 p-3 rounded-lg border border-gray-200 dark:border-[#2B3139] shadow-sm hover:border-yellow-500/50 dark:hover:border-[#F0B90B]/50 transition-colors duration-200">
                                            <div className="flex items-center justify-between border-b border-gray-200 dark:border-[#2B3139] pb-2">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="font-bold text-sm text-gray-900 dark:text-[#EAECEF]">{displaySymbol}</span>
                                                    <span className="text-[10px] text-gray-500 dark:text-[#848E9C] bg-gray-100 dark:bg-[#2B3139] px-1 rounded">
                                                        {leverage}X
                                                    </span>
                                                    <span className="text-[10px] text-gray-900 dark:text-[#EAECEF] bg-gray-100 dark:bg-[#2B3139] px-1 rounded" title="Total Position Value">
                                            ...
                                            <div className="grid grid-cols-4 gap-2 text-[10px] mb-2">
                                                <div>
                                                    <p className="text-gray-500 dark:text-[#848E9C]">Entry</p>
                                                    <p className="font-mono text-gray-900 dark:text-[#EAECEF]">{pos.entry_price.toFixed(2)}</p>
                                                </div>
                                                <div>
                                                    <p className="text-gray-500 dark:text-[#848E9C]">Mark</p>
                                                    <p className="font-mono text-gray-900 dark:text-[#EAECEF]">{pos.mark_price ? pos.mark_price.toFixed(2) : '-'}</p>
                                                </div>
                                                <div>
                                                    <p className="text-gray-500 dark:text-[#848E9C]">SL</p>
                                                    <p className="font-mono text-red-600 dark:text-[#F6465D]">{pos.sl_price ? pos.sl_price.toFixed(2) : '-'}</p>
                                                </div>
                                                <div>
                                                    <p className="text-gray-500 dark:text-[#848E9C]">TP</p>
                                                    <p className="font-mono text-green-600 dark:text-[#0ECB81]">{pos.tp_price ? pos.tp_price.toFixed(2) : '-'}</p>
                                                </div>
                                            </div>
                                            <div className="text-[10px] border-t border-gray-200 dark:border-[#2B3139] pt-2 flex justify-between items-center">
                                                <span className="text-gray-500 dark:text-[#848E9C]">Entry Time</span>
                                                <span className="font-mono text-gray-900 dark:text-[#EAECEF]">
                                                    {pos.entry_time 
                            ...
                            <div className="flex items-center justify-center h-16 text-gray-500 dark:text-[#848E9C] gap-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {/* Recent Trades */}
                        <div className="bg-white dark:bg-[#1E2329] rounded-xl border border-gray-200 dark:border-[#2B3139] overflow-hidden h-[400px] flex flex-col shadow-md">
                                    ...
                                    <tbody className="divide-y divide-gray-200 dark:divide-[#2B3139]">
                                        {trader?.trade_history && trader.trade_history.length > 0 ? (
                                            trader.trade_history.map((trade) => {
                                                ...
                                                const sideColor = positionSide === 'LONG' ? 'text-green-600 dark:text-[#0ECB81]' : 'text-red-600 dark:text-[#F6465D]';
                                                ...
                                                return (
                                                    <tr key={trade.id} className="hover:bg-gray-100 dark:hover:bg-[#2B3139]/30 transition-colors">
                                                        ...
                                                        <td className="px-3 py-2 text-right font-mono text-gray-900 dark:text-[#EAECEF]">
                                            ...
                                            <tr>
                                                <td colSpan={9} className="px-3 py-8 text-center text-gray-500 dark:text-[#848E9C]">
                        ...
                        <div className="bg-white dark:bg-[#1E2329] rounded-xl border border-gray-200 dark:border-[#2B3139] overflow-hidden h-[400px] flex flex-col shadow-md">
                            <div className="p-4 border-b border-gray-200 dark:border-[#2B3139] flex items-center justify-between bg-white dark:bg-[#1E2329]">
                                <h3 className="font-semibold text-sm flex items-center gap-2 text-gray-900 dark:text-[#EAECEF]">
                                    <Clock className="w-4 h-4 text-yellow-600 dark:text-[#F0B90B]" />
                                ...
                                <div className="flex gap-2">
                                    <input 
                                        ...
                                        className="bg-gray-100 dark:bg-[#2B3139] text-xs text-gray-900 dark:text-[#EAECEF] px-2 py-1 rounded border border-gray-300 dark:border-[#474D57] focus:outline-none focus:border-[#F0B90B] w-24"
                                        onChange={(e) => setSignalSearch(e.target.value)}
                                    />
                                    <select 
                                        className="bg-gray-100 dark:bg-[#2B3139] text-xs text-gray-900 dark:text-[#EAECEF] px-2 py-1 rounded border border-gray-300 dark:border-[#474D57] focus:outline-none focus:border-[#F0B90B]"
                                        ...
                                        {signalsError ? (
                                            <tr>
                                                <td colSpan={8} className="px-3 py-8 text-center text-red-600 dark:text-[#F6465D]">
                                                    {signalsError} <br/>
                                                    <button onClick={() => fetchStrategySignals()} className="mt-2 px-3 py-1 bg-gray-100 dark:bg-[#2B3139] rounded text-xs hover:bg-gray-200 dark:hover:bg-[#363C45]">Retry</button>
                                                </td>
                                            </tr>
                                        ) : signalsLoading ? (
                                            <tr>
                                                <td colSpan={8} className="px-3 py-8 text-center text-gray-500 dark:text-[#848E9C]">
                                                    Loading signals...
                                                </td>
                                            </tr>
                                            ...
                                            <tr>
                                                <td colSpan={8} className="px-3 py-8 text-center text-gray-500 dark:text-[#848E9C]">No matching signals</td>
                                            </tr>
                                        ) : (
                                            filteredSignals.map((signal, index) => {
                                                ...
                                                return (
                                                    <tr key={`${signal.symbol}-${index}`} className="hover:bg-gray-100 dark:hover:bg-[#2B3139]/30 border-b border-gray-200 dark:border-[#2B3139]/50">
                                                        ...
                                                        <td className="px-3 py-2">
                                                            <div className={`flex items-center gap-1 ${isUp ? 'text-green-600 dark:text-[#0ECB81]' : (isDown ? 'text-red-600 dark:text-[#F6465D]' : 'text-gray-500 dark:text-[#848E9C]')}`}>
                                                        ...
                                                        <td className={`px-3 py-2 font-mono ${indicators?.rsi && (indicators.rsi > 70 || indicators.rsi < 30) ? 'text-yellow-600 dark:text-[#F0B90B]' : 'text-gray-900 dark:text-[#EAECEF]'}`}>
                                                            {indicators?.rsi ? indicators.rsi.toFixed(1) : '-'}
                                                        </td>
                                                        <td className={`px-3 py-2 font-mono ${indicators?.adx && indicators.adx > 25 ? 'text-yellow-600 dark:text-[#F0B90B]' : 'text-gray-500 dark:text-[#848E9C]'}`}>
                                                            {indicators?.adx ? indicators.adx.toFixed(1) : '-'}
                                                        </td>
                                                        <td className={`px-3 py-2 font-mono ${indicators?.macd_hist && indicators.macd_hist > 0 ? 'text-green-600 dark:text-[#0ECB81]' : (indicators?.macd_hist && indicators.macd_hist < 0 ? 'text-red-600 dark:text-[#F6465D]' : 'text-gray-900 dark:text-[#EAECEF]')}`}>
                                                            {indicators?.macd_hist ? indicators.macd_hist.toFixed(2) : '-'}
                                                        </td>
                                                        <td className="px-3 py-2 text-right font-mono text-gray-900 dark:text-[#EAECEF]">
                                                            {signal.price.toLocaleString()}
                                                            {indicators?.ema && <span className="text-[10px] text-gray-500 dark:text-[#848E9C] block">EMA:{indicators.ema.toFixed(0)}</span>}
                                                        </td>
                                                        <td className="px-3 py-2 text-xs text-gray-500 dark:text-[#848E9C] max-w-[200px] truncate" title={signal.strategy_result?.reason}>
